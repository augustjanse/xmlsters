...
class ViewTestCase(TestCase):
    ...
    def test_can_get_chart(self):
        ...
        response = self.client.get(
            reverse('details', kwargs={'placement': chart.placement}), format="json"
        )
    ...
    def test_can_update_chart(self):
        ...
        response = self.client.put(
            reverse('details', kwargs={'placement': Chart.objects.get().placement}),
            chart, format='json'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_delete_chart(self):
        chart = Chart.objects.get()
        response = self.client.put(
            reverse('details', kwargs={'placement': chart.placement}),
            chart, format='json', follow=True
        )
